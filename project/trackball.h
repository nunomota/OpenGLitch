...
public:
    ...
    mat4 Drag(float x, float y) {
      ...
      // article.
      vec3 rotAxis = normalize(cross(anchor_pos_,current_pos));

      float dot_product = dot(normalize(anchor_pos_),normalize(current_pos));
      float magnitude = acos(dot_product) * 2.0;  // some extra scaling

      rotation = rotate(rotation,magnitude,rotAxis);
...
private:
    ...
    void ProjectOntoSurface(vec3& p) const {
      ...
      // check whether to use hyperbolic sheet or sphere for z calculation

      if((pow(p[0],2) + pow(p[1],2) >= (pow(radius_,2)/2))){
        // hyperbolic sheet
        p[2] = ( pow(radius_,2) / 2) / (abs(sqrt(pow(p[0],2) + pow(p[1],2))));

      }
      else{      

        p[2] = abs(sqrt(pow(radius_,2)-pow(p[0],2)-pow(p[1],2)));
